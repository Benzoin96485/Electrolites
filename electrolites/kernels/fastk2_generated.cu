...
k2_2021(KARGS2)
{
    __shared__ double s_aij[36];
    __shared__ double s_iaij[36];
    __shared__ double s_ajaij[36];
    __shared__ double s_xij[36];
    __shared__ double s_yij[36];
    __shared__ double s_zij[36];
...
while (pair_ij < npairs_ij) {
    ...
    for (int task_id = sq_id; task_id < ntasks+sq_id; task_id += nsq_per_block) {
        ...
        for (int klp = 0; klp < kprim*lprim; ++klp) {
            ...
            for (int ijp = 0; ijp < iprim*jprim; ++ijp) {
                ...
                if (gout_id == 0) {
                    ...
                    gx[nsq_per_block*g_size] = cicj * iaij * iakl * inv_s * coef0;
                }
                double theta = aij * akl * inv_s2;
                double rr = xpq * xpq + ypq * ypq + zpq * zpq;
                rys_roots_tab<3>(theta * rr, rw, nsq_per_block, gout_id, blockDim.y, tab);
        ...
        if (task_id < ntasks) {
            ...
            int l0 = ao_loc[lsh];
            double val;
            for (int i_dm = 0; i_dm < 1; ++i_dm) {
                double *dm = dm_all + i_dm * nao * nao;
                double *vk = vk_all + i_dm * nao * nao;
                switch (gout_id) {
                case 0:
                val = 0;
