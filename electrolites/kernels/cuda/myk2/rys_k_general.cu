// Generated by gen_kgeneral.py from GPU4PySCF's gvhf-rys/rys_contract_k.cu.
                   ...
                   int *head, int queue_depth, double inv_om2)
{
    ...
    double *s_zij   = cicj_cache + 6*iprim*jprim;
...
#pragma unroll
        ...
        for (int klp = 0; klp < kprim*lprim; ++klp) {
            ...
            if (gout_id == 0) {
                ...
                akl_cache[2*nsq_per_block] = iakl;
            }
                        ...
                        double *env)
{
    ...
    // GPU4PySCF 1.7.0, which this kernel was copied from, packed this mask
    // with '>>' instead of '<<' (upstream #791, fixed in 1.8.0).  The
    // right-shifts collapse every bit to zero except the ll one, so the
    // switch below only ever reached inner_dot<3,3,3,3> or <3,3,3,1> -- the
    // fully general instantiations -- and the cheaper <1,...> specialisations
    // for an s shell in i/j/k were never selected.
