...
res = {'tag': tag, 'gpu4pyscf': gpu4pyscf.__version__, 'xyz': a.xyz,
       ...
       'natm': mol.natm, 'nao': int(mol.nao), 'cycles': a.cycles,
...
# the gradient, on the same density.  Its unpatched half spends real time in
# GPU4PySCF's CPU nuclear-attraction derivative (which `fastgradh` moves to
# the GPU), so an unset OMP_NUM_THREADS makes the baseline look far worse than
# it is -- set it explicitly, or pass --no-grad.
if not a.no_grad:
